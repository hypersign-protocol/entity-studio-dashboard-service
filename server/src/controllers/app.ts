...
const validateApp = async (req: Request, res: Response) => {
  try {
    ...
    if (appInDb.appId === appId && appInDb.appSecret === appSecret) {
      jwt.sign(appInDb, jwtSecret, { expiresIn: jwtExpiryInMilli }, (err, token) => {
...
const login = (req: Request, res: Response) => {
  ...
  try {
    ...
    jwt.sign(param, jwtSecret, { expiresIn: jwtExpiryInMilli }, (err, token) => {
      if (err) throw new Error(err);
...
export { registerApp, validateApp, login, getAppList, getOne };
